...
export const managedPage: ServicePageBilingual = {
  ...
  ru: {
    h1: "Управляемый подряд под KPI",
    subtitle:
      "Сдвигаем фокус с «человеко-часов» на результат: план, исполнение, отчётность и улучшения по согласованным метрикам для зрелых распределительных центров и производственных линий.",
    intro: [
      "Управляемый подряд подходит, когда операции уже измеримы: есть данные из WMS, понятные целевые показатели по явке, ошибке и простоям, и готовность к еженедельным разборам с подрядчиком.",
      "Мы совместно фиксируем KPI, дорожную карту на 90 дней и формат сводки: явка, замены, инциденты и себестоимость смены — в одной плоскости для операционного директора и финансов.",
      "Смысл модели: не «ещё отчёты в почте», а владелец сценария улучшения — что меняем на следующей неделе, если тренд не тот, с привязкой к источнику данных, а не к фразе «срочно дайте людей».",
    ...
    includes: [
      { name: "Операционный контакт на объекте / дистанционное сопровождение контракта", included: true },
    ...
    faq: [
      { q: "Кому подходит управляемый подряд?", a: "Зрелым операциям с понятными метриками и готовностью к совместным разборам раз в неделю." },
      { q: "Какие гарантии по KPI?", a: "Фиксируются в приложении к договору; штрафные и бонусные схемы — индивидуально." },
      { q: "Можно ли совместить с подрядом по складским сменам?", a: "Да, управляемый подряд часто вырастает из уже стабильного контракта на смены." },
      { q: "Сколько длится внедрение?", a: "Зависит от аудита процессов; ориентир обсуждается в КП." },
      { q: "Нужен ли ограниченный старт?", a: "Часто да — первые смены на участке или по части линии, чтобы откалибровать метрики до полного масштаба." },
  ...
  en: {
    ...
    includes: [
      { name: "On-site operations contact / remote contract support", included: true },
