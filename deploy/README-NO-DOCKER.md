...
| `scripts/yandex-metrica.mjs` | CLI: Management API — список счётчиков, карточка счётчика, **проверка целей** vs `analytics-yandex.ts` |
...
1. **Счётчик на сайте:** создай на [metrika.yandex.ru](https://metrika.yandex.ru/), домен = прод. В **`.env.production`** на VPS (и локально для проверки) задай **`NEXT_PUBLIC_YANDEX_METRICA_ID=<номер>`**, затем **`npm run build:vps`** — иначе ID не попадёт в клиентский бандл. Тег и цели после согласия cookies — `src/components/seo/yandex-metrika.tsx`, `src/lib/analytics-yandex.ts`; список целей — **`my-guide/YANDEX-METRIKA-GOALS.md`** (создай цели в интерфейсе Метрики с теми же id).
2. **API Метрики (локально):** в приложении [oauth.yandex.ru](https://oauth.yandex.ru/) добавь право **«Яндекс.Метрика»** (чтение). Затем снова **`node scripts/yandex-webmaster.mjs auth-open`** → **`exchange-file`**, чтобы access_token включал **metrika:read** (или положи токен в **`deploy/.metrica-oauth.local.env`** — см. **`deploy/metrica-oauth.env.example`**).
3. Команды из корня `apv-b2b-landing/`:
   - `node scripts/yandex-metrica.mjs counters` — список счётчиков аккаунта.
   - `node scripts/yandex-metrica.mjs counter` — детали счётчика из `NEXT_PUBLIC_YANDEX_METRICA_ID` / `YANDEX_METRICA_COUNTER_ID`.
   - `node scripts/yandex-metrica.mjs goals-check` — есть ли в счётчике JS-цели из кода (`lead_form_submit`, …).
