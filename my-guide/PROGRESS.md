...
**2026-05-16 (вечер; обратная связь заказчика — фото 01/04 + чистка блога):** **Главная**: тайлы **01 «Зона хранения»** и **04 «Производство · отгрузка»** заменены на **пользовательские кадры** (присланы заказчиком; РЦ с разметкой пола / отгрузка с гидротележкой и погрузчиком), `scripts/_resize-real-photos.mjs` подхватывает второй источник `~/.cursor/projects/.../assets/`, ресайз 1280×960 / mozjpeg Q80; `industrialBand.photo1Alt` и `photo4Alt` в `ru.json` переписаны под новые сцены. **Блог**: системно снят GPT-маркер **«…, не «X»»** в кавычках — `blog-published.ts` 31 → **10** органичных контрастов; `cross-priority-narratives.ts` 9 → **1**; одновременно убран жаргон **ЧП** (численность персонала, читался как «чрезвычайное происшествие») × 5 → 0. Подробности — **`COPYWRITER-FINAL-AUDIT-REPORT.md`** часть IX.

**2026-05-09 (mobile hero SLA):** **`HeroSlaDashboard`** — `min-w-0` / `w-full` на фигуре, компактнее padding на xs, блок графика **`overflow-hidden`**, подписи throughput — колонкой на мобилке; **`hero-section`** — **`min-w-0`** на колонках грида; **`CaseSparkline`** — **`min-w-0`** на SVG. Hero — **`overflow-x-clip`**.
...
**2026-04-23 (роли 04+05, редакторский SEO):** выровнена лексика **ворот/экспедиции** в программатике (`cross-priority-narratives.ts`, `programmatic-city-local.ts`, `programmatic-longread.ts`), коммерции (`commercial-editorial.ts`), кейсах (`cases-stub.ts`), блоге (`blog-published.ts`, заголовок статьи про ворота без «слоты» в H1, **slug не меняли**), страницах услуг (`autsorsing`, `nochnye-smeny`, `migration` data). **Блог-хаб/рубрики:** карточки и **ItemList** JSON-LD только из русских `title`/`excerpt` (`blogCardFields`). См. **`my-guide/COPYWRITER-FINAL-AUDIT-REPORT.md`** — часть V.
