...
**2026-05-16 (поздний вечер; одна большая волна — P3 + юр-аудит по 152-ФЗ/ГК/38-ФЗ + 03+05):** Косметика P3: удалён мёртвый код (`geoHubEditorial/Region/City` + GEO_HUB/REGION/TAIL + неиспользуемые импорты), вычищен последний «не «X»» в `programmatic-city-local.ts` (Одинцово). **Юр-аудит** (15 правок): дата редакции `LEGAL_VERSION_DATE` + строка версии в каждом юрдоке; в Политику конфиденциальности добавлены ссылки на ст. 18.1 / 22 / 22.1 / 19 152-ФЗ + ПП 1119/2012, лицо ответственное за обработку ПДн, конкретика мер защиты, место БД ПДн на территории РФ; в Согласие — категории получателей (хостинг, IT, аналитика, антибот, почта, нейросеть, банк), правовое основание (п. 1 ч. 1 ст. 6 + ч. 1 ст. 9), отзыв по ст. 21, подтверждение 18+; в Оферту — сужение адресатов до B2B/ИП с явным выводом из-под 2300-1, ссылка на ст. 437–438 ГК, конкретизация налогового режима (ст. 346.11 НК для УСН), оговорка про «информационные» цены сайта (не оферта в смысле ст. 437); в Правила — ссылка на 149-ФЗ + блок про маркировку рекламы (38-ФЗ ст. 18.1 — на собственном сайте не реклама по разъяснениям ФАС). **03+05** проход: «телодвижения» в кавычках → нейтральный B2B-стиль; опечатка «около на пятую» → «примерно на пятую часть»; **главное** — исправлено «Софино» → «Софьино» (правильное название НП Раменского округа МО) в кейсах + EN-транслитерация. Подробности — **`COPYWRITER-FINAL-AUDIT-REPORT.md`** часть X.
...
**2026-05-18 (снят раздел «Площадки»):** удалены маршруты **`/ploshchadki`** и контент под чужие бренды маркетплейсов; навигация и `commercial-editorial` вычищены; старые URL **301 → `/ru/otrasli/sklady-e-commerce`**. Документация (`ROUTES-VS-MASTER-PROMPT`, `COMPOSER-2`, роадмап) синхронизирована с фактом.
...
| 5 | **Яндекс.Метрика + Вебмастер (CLI):** прод `NEXT_PUBLIC_YANDEX_METRICA_ID` + сборка; `metrica:goals-check` / при необходимости `goals-install` (OAuth **metrika:write**); быстрый чеклист в **`.cursor/rules/04-seo-strategist-orchestrator.mdc`**; GEO: **`/llms.txt`** + AI user-agent в **`robots.ts`** | [x] |
